# Code in Style {#sec-code-best-practice}
...
Don't worry if this all seems like a lot to take in though, because in @sec-code-advcd-best-practice we'll find out how to get the computer to apply style to your code *automatically*.
...
Learning *all* of the PEP8 conventions would be tedious beyond belief. The good news is that a combination of packages and Visual Studio Code will do a lot of them for you. Packages can re-style and even re-write your code automatically (more on that in Chapter on @sec-code-advcd-best-practice).
...
There are packages that can re-organise your whitespace for you; these are featured in the Chapter on @sec-code-advcd-best-practice.
...
```python
def function(input_one, input_two, input_three, input_four):
    result = (input_one, +input_two, +input_three, +input_four)
    return result
```

Again, automatic formatters, which we'll meet in the @sec-code-advcd-best-practice Chapter, will try and break up long lines for you.
...
::: {.callout-note title="Start interactive windows and terminals within your project directory"}
