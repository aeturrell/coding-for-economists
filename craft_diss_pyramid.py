...
def plot_pyramid():
    ...
    blog = mpatches.RegularPolygon([base_blog_triangle, start_y], 3, height, alpha=0.4, orientation=np.pi)
    ax.add_patch(diss)
    ax.add_patch(blog)
    ...
    ax.annotate(
        txtwrp.fill("Inverted pyramid of dissemination", 30),
    ...
    ax.annotate(
        txtwrp.fill("Typical structure of a blog post", 30),
